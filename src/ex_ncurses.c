...
ex_poll(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    int rc = enif_select(env, STDIN_FILENO, ERL_NIF_SELECT_READ, data->resource, NULL, data->atom_undefined);
...
ex_stop(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    int rc = enif_select(env, STDIN_FILENO, ERL_NIF_SELECT_STOP, data->resource, NULL, data->atom_undefined);
    return rc >= 0 ? data->atom_ok : make_error(env, "enif_select");
}
